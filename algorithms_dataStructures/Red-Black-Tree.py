...
class Node():
    ...
    def __repr__(self):
        return f'{self.color} {self.data} Node'
...
class Employee:
    ...
    def __init__(self, registrationNumber,departmenId,phoneNumber):
        self.registrationNumber=registrationNumber
        self.departmenId=departmenId
        self.phoneNumber=phoneNumber

if __name__ == "__main__":

    emp_list=[]
    example_list=[[1010,34,4342],[1001,51,6467],[1002,43,2359],[1008,21,6761],[2001,45,2345],[2006,23,68,62]]
    for i in example_list:
        emp_list.append(Employee(i[0],i[1],i[2]))
    
    # print(emp_list[0])
    rbt=RedBlackTree()
    for i in range(len(emp_list)):
        rbt.insert(int(emp_list[i].registrationNumber))
    rbt.pretty_print()

    while True:
        registrationNo=int(input("Enter Employee's Registration Number: "))

        try:
            rbt.searchTree(registrationNo)
            for i in range(len(emp_list)):
                if int(emp_list[i].registrationNumber) == registrationNo :
                    print("Department id is : {} . Internal phone number is {}".format(emp_list[i].departmenId,emp_list[i].phoneNumber))
        except:
            print("Employee not found !")
            key=input("Do you want to add this employee to system (y/n)?")
            if key == 'y' or key== 'Y':
                depertmentID, phoneNo = input("What is Department id and Internal phone number?: ").split()
                emp_list.append(Employee(registrationNo,depertmentID,phoneNo))
                rbt.insert(int(registrationNo))
